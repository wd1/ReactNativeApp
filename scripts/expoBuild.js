  ...
  return new Promise((resolve, reject) => {
    ...
    runCommand(command, (output) => {
      if (output.includes("What's your Apple ID")) {
        reject(new Error('No apple credentials, run "npm run build:ios" to enter credentials'));
      }
    })
      .then(resolve)
      .catch(reject);
...
function checkStatus() {
  return new Promise((resolve, reject) => {
    runCommand('yarn run build:status', (output) => {
      ...
      .then(() =>
        reject(new Error('Unexpected response, check it and add to build')))
      .catch(reject);
  });
}
...
function uploadBuild(filepath) {
  return new Promise((resolve, reject) => {
    const command = `fastlane pilot upload -u ${process.env.APPLE_ID} -q ${
      process.env.FASTLANE_TEAM_ID
    } -i ${filepath}`;
...
if (ios && !android) {
  ...
  throw new Error('Both android and ios tags were found, please check the build script');
}
